...
6. 目录扫描现在有 4096 文件、512 MiB 总量和 15 秒耗时上限；同步扫描尚未支持中途取消，下一切片再引入任务令牌。
...
- 目录扫描设置页数、总字节数和总耗时配额；当前已实现文件数/总字节数/15 秒耗时上限，取消令牌留待下一切片；
- 重新关联预览把“超过时间上限”与普通读取失败分开提示，超时或失败都不会修改旧目录和旧缓存；
- 提交 [eb520ff4fd88a992f5b3ccdd10db1443c90a95eb](https://github.com/TttXxx36/Open-reader-desktop/commit/eb520ff4fd88a992f5b3ccdd10db1443c90a95eb) 对应 CI run [31378331544](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/31378331544) 已通过前端、Rust fmt/check/tests、TXT 性能证据和 Windows 编译/峰值 RSS 采样。
