...
M6.0 只有在 P0 缺陷全部关闭、前端契约检查和 Rust 测试通过、并在 Windows 安装包上完成手工验收后，才进入 M6.1。M6.1 不以“能解析 JSON”为通过标准，而以“用户能看到兼容性边界并安全导入”为通过标准。M6.3/M6.4 的代码任务及远程 CI 验证已完成；在 Windows 安装包手工验收前，不宣称 M6 整体完成。下一步是下载 GitHub Actions 生成的 Windows 安装包/便携版，按本节验收清单进行手工回归，不在本地构建或安装。
