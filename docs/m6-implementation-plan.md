...
- [x] 将首页概览与阅读设置抽成独立视图组件；书源/阅读器的业务状态仍由 App 编排，后续继续拆分
...
M6.0 只有在 P0 缺陷全部关闭、前端契约检查和 Rust 测试通过、并在 Windows 安装包上完成手工验收后，才进入 M6.1。M6.1 不以“能解析 JSON”为通过标准，而以“用户能看到兼容性边界并安全导入”为通过标准。M6.3/M6.4 的代码任务已完成；在 Windows 安装包手工验收前，不宣称 M6 整体完成，下一步优先等待 GitHub Actions 验证并修复回归。
