...
- 本地阅读器已显示经过同一安全过滤的“本章内部链接”索引，便于验收 EPUB 目录/跳转数据；不会主动访问任何链接。
- GitHub Actions run [31325922734](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/31325922734)：前端检查、Rust fmt/check 和 86 个 Rust 测试通过。
- 剩余工作：片段定位与跨章节跳转、有限 CSS 白名单、标题层级/缺失资源提示，以及损坏 container/OPF/manifest/spine 的恢复夹具。
