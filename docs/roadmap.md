...
- M9.2.3/M9.2.4/M9.2.5（目录恢复、状态反馈、超时边界与取消反馈已完成首切片）：原生目录选择、相对路径/文件名+大小候选、missing/stale/needs_relink 差异预览、事务化重新关联，仅针对 stale 页的有界 SHA-256 复核、书架/阅读器缺页提示，以及 15 秒扫描超时和可取消任务令牌；摘要一致可恢复为 ready，无摘要旧页继续保持 stale。下一步完成真实 Windows 手工验收，再推进 M9.3。
