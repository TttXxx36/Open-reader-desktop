...
- **M7.5 书源诊断**：单源重试、搜索/书籍/章节失败历史、本地保留与统计、跨请求 `operation_id`、本地失败报告、有限原因分类、旧库升级夹具、报告 schema_version 兼容约定、`source_metrics` 请求/缓存观测摘要和规则执行指标已完成；SQLite 0010 按书源/阶段统计网络请求、成功/失败次数和新鲜缓存命中，SQLite 0011 按来源/阶段/规则键统计规则解析 attempts、success/no-match/failure，并在书源页与诊断报告展示明确分母和 `observed` 状态。规则指标四阶段、缓存/取消边界和隐私约定详见 [规则执行指标边界](source-rule-metrics.md)；CI run [31320769641](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/31320769641) 已通过前端检查、Rust fmt/check 和 76 个 Rust 测试。下一步补充字段级规则键、skipped 的可见展示和授权合成边界夹具，再进入 M8。
...
8. M7.5 单源重试、搜索/书籍/章节失败历史、本地保留与统计、跨请求关联 ID、本地失败报告、有限原因分类、旧库升级、报告 schema_version 兼容、source_metrics 请求/缓存统计和规则执行指标首个实现切片已完成（SQLite 0010/0011、书源页与报告展示，CI run [31320769641](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/31320769641)，76 个 Rust 测试）；下一步补充字段级规则键、skipped 可见展示和授权合成边界夹具，再进入 M8。
