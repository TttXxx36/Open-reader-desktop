...
- **M7.1 书源管理基础（已完成 M7.1a 基础切片，增强项待续）**：SQLite 已保存分组、类型、权重、发现开关、自定义顺序和备注；列表已支持按分组/自定义顺序/权重排序、分组筛选、启用/停用、发现开关和元数据编辑。M7.1b 再补批量操作、拖拽重排、冲突 diff 与回滚。
