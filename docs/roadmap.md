...
- **M7.1 书源管理基础（已完成 M7.1a + M7.1b 首个切片，回滚待续）**：SQLite 已保存分组、类型、权重、发现开关、自定义顺序和备注；列表已支持按分组/自定义顺序/权重排序、分组筛选、多选、批量启停/发现开关/删除、同分组上下移动、导入新增/更新/无变化差异提示。M7.1c 再补跨分组拖拽、批量元数据、冲突策略、快照回滚和失败原子性。
- **M7.2 规则执行增强**：分页变量、模板变量、链式规则、更多 JSONPath 安全表达式和规则调试快照；每项能力都必须有夹具和超时/大小上限。
...
4. M7.1c 跨分组拖拽、批量元数据、冲突策略、快照回滚和失败原子性。
5. M7.2 分页/变量/调试快照。
5. 并行等待 M6.5 Windows Actions 权限恢复；权限恢复后执行安装/升级/卸载/WebView2 手工验收。
6. M8、M9、M10、M11、M12 按依赖顺序推进；每阶段完成一次路线图复盘。
