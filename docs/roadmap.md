...
- **M7.3 XPath 评估（静态识别首个切片已完成）**：导入预览在规则上下文中识别 `//`、`xpath:`、`xpath=`、`@xpath` 等表达式，最多保留 8 条、每条 512 字节并展示原始值和不执行原因；预览复用真实安全校验，当前仍不解析、不执行、不降级为 CSS。下一步仅做合成 HTML 离线解析 PoC 和覆盖/资源指标。
...
6. M7.3 XPath 静态识别/导入预览闸门（已完成，CI run 30769462298 通过）；下一步仅做合成 HTML 离线解析 PoC 与覆盖率/资源指标，不进入真实网络执行。
