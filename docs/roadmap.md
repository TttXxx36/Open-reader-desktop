...
- **M7.5 书源诊断**：单源重试、本机搜索失败历史和书籍/章节失败历史首个切片已完成（`retry_source_search`、失败项“重试此源”入口、按源替换结果/诊断、SQLite 0008、书源页筛选/刷新/清空、book/chapter 阶段记录，CI runs 31309951094、31310413566 的 71 个 Rust 测试通过）；跨请求关联 ID、重试原因细分、导出版本迁移和本地统计仍待后续切片。
...
8. M7.5 单源重试、搜索失败历史和书籍/章节失败历史首个切片已完成（CI runs 31309322671、31309951094、31310413566）；下一步补跨请求关联 ID、重试原因细分和本地统计，再进入 M8。
