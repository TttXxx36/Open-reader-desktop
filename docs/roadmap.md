...
5. M7.2 分页/变量/调试快照首个切片、分页诊断、安全回退链子集、缓存命中提示、有限 JSONPath、每阶段最多 8 项候选的 URL 回退链与脱敏诊断快照导出（已完成，CI run 30760156272、30760475594、30761456593、30761886043、30763032154、30764314398、30764884266 通过）；下一步推进书籍/章节缓存的统一重试时间线、复杂规则的明确拒绝与请求级取消/超时预算。
