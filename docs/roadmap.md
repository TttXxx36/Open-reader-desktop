...
- **M7.5 书源诊断**：单源重试与本地失败历史首个切片已完成（`retry_source_search`、失败项“重试此源”入口、按源替换结果/诊断、SQLite 0008 失败摘要、书源页筛选/刷新/清空，CI run 31309951094 的 71 个 Rust 测试通过）；书籍/章节失败历史、跨请求关联、重试原因细分、导出版本迁移和本地统计仍待后续切片。
...
8. M7.5 单源重试与搜索失败历史首个切片已完成（CI run 31309322671、31309951094）；下一步补书籍/章节失败历史、跨请求诊断关联、重试原因细分和本地统计，再进入 M8。
