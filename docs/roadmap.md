...
- **M7.5 书源诊断**：单源重试、搜索/书籍/章节失败历史、本地保留与统计、跨请求 `operation_id` 和本地失败报告首个切片已完成；书源页显示任务 ID并支持导出最近 64 条脱敏失败摘要（256 KB 上限、`schema_version: 1`），失败原因已细分为 10 类有限码并有回归夹具，CI run 31313787317 的前端检查、Rust fmt/check 和 72 个 Rust 测试通过；诊断/报告版本迁移和后续兼容性指标进入 M7.5f。
...
8. M7.5 单源重试、搜索/书籍/章节失败历史、本地保留与统计、跨请求关联 ID、本地失败报告和有限原因分类首个切片已完成（CI runs 31309322671、31309951094、31310413566、31311000040、31311905146、31313787317）；下一步补诊断/报告版本迁移，再进入 M8。
