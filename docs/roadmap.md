...
9. M8.5.5a/b/c/d1/e1-e4 已完成（CI run 31359196211 通过）；下一步进入 M9 图片序列数据库化与书架恢复计划，先冻结 schema、文件变更检测、失效提示和恢复测试，再按依赖顺序推进 M10、M11、M12，并在每阶段完成一次路线图复盘。
