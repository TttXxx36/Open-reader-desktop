...
- **M7.3 XPath 评估**：先做只读 XPath 解析器 PoC、兼容性统计和资源配额；不能因为“解析成功”就默认放开真实网络执行。
...
6. 并行等待 M6.5 Windows Actions 权限恢复；权限恢复后执行安装/升级/卸载/WebView2 手工验收。
7. M8、M9、M10、M11、M12 按依赖顺序推进；每阶段完成一次路线图复盘。
