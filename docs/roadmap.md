...
5. M7.2 分页/变量/调试快照首个切片、分页诊断、安全回退链子集、缓存命中提示、有限 JSONPath、每阶段最多 8 项候选的 URL 回退链、脱敏诊断快照导出、书籍/章节统一请求步骤、URL 阶段超时预算、pipeline 总耗时预算、相对可排序时间线、端到端/远端/多源取消、多源失败聚合与受限 next URL 中间模型（已完成，CI run 30760156272、30760475594、30761456593、30761886043、30763032154、30764314398、30764884266、30765352024、30765823280、30766330200、30766847254、30767351427、30767940334、30768670672 通过）；下一步评估自动追链、深度/响应配额与更丰富的跨请求时间元数据。
