...
- **M7.5 书源诊断**：单源重试、搜索/书籍/章节失败历史、本地保留与统计首个切片已完成（`retry_source_search`、失败项“重试此源”入口、SQLite 0008、book/chapter 阶段记录、512 条保留上限、按原因/阶段统计，CI runs 31309951094、31310413566、31311000040 的 71 个 Rust 测试通过）；跨请求关联 ID、重试原因细分、导出版本迁移和本地报告导出仍待后续切片。
...
8. M7.5 单源重试、搜索/书籍/章节失败历史及本地统计首个切片已完成（CI runs 31309322671、31309951094、31310413566、31311000040）；下一步补跨请求关联 ID、重试原因细分和本地报告导出，再进入 M8。
