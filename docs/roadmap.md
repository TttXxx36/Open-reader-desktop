...
- **M7.3 XPath 评估（静态识别、离线 PoC 与首轮夹具已完成）**：导入预览在规则上下文中识别 `//`、`xpath:`、`xpath=`、`@xpath` 等表达式，最多保留 8 条、每条 512 字节并展示原始值和不执行原因；离线 PoC 只解析受限路径/谓词并统计合成 HTML 节点与估算工作量，首轮夹具覆盖 6 条受限语法和 6 条拒绝语法，明确拒绝函数、联合、轴、父节点和超限输入，不翻译为 CSS、不访问网络。下一步补充授权合成夹具和解析耗时分布。
...
6. M7.3 XPath 静态识别、离线解析 PoC 与首轮语法夹具（已完成，CI run 30769462298、30770030409、30770432561 通过）；下一步补充授权合成夹具与解析耗时分布，继续禁止真实网络执行。
