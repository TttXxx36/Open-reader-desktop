...
> 当前候选（2026-08-21）：PR18 `0.2.0` Windows Release [32402250634](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/32402250634) 与 installer smoke [32403363944](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/32403363944) 已通过；Computer Use 手工验收记录见 [windows-manual-acceptance-2026-08-21.md](windows-manual-acceptance-2026-08-21.md)。M9.3.1-d2 的 `0016` 迁移、纯文本单事务合并、生命周期过滤和回滚夹具已由 CI [32463202309](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/32463202309) 验证。随后完成 M7 P1 两轮安全切片（item 自身节点匹配、响应字符集识别、目录章节链接回退、正文安全 CSS 回退），CI [32466122037](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/32466122037)、[32468240226](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/32468240226) 全部通过；下一步补授权夹具、乱码降级和字段级规则差异诊断，再进入 M9.3.1-d3。
...
详细执行清单见 [M9 图片序列数据库化与书架恢复计划](m9-image-sequence-plan.md)。M9.0—M9.3.1-d2 已完成 SQLite 模型、图片序列恢复、元数据、重复预览和纯文本事务合并；d2 使用 `0016` 的 lifecycle/operation/item/alias 表，普通书架与重复查询过滤 `active`，来源书籍改为 `merged` 但不物理删除；CI [32463202309](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/32463202309) 已通过。d3 撤销、旧 ID 单跳跳转和别名环检测仍未开始。
...
1. **P1 书源兼容性收尾**：两轮安全切片已完成编码识别、`item` 自身节点匹配、目录章节链接回退和正文安全 CSS 回退（CI 32466122037、32468240226）；继续针对手工验收发现的乱码标题和异常结果，补充授权响应夹具、降级展示与字段级 `toc/content` 诊断；继续明确拒绝 XPath/脚本执行。
