...
- **M7.1 书源管理基础**：SQLite 保存分组、类型、权重、发现开关、排序和备注；列表按分组/权重/自定义顺序排序，支持启用/停用、批量操作和可解释状态。
...
1. M7.0 元数据保真（本轮代码与 CI）。
2. M7.1 书源管理基础（SQLite + UI + 迁移测试）。
3. M7.2 分页/变量/调试快照。
4. 并行等待 M6.5 Windows Actions 权限恢复；权限恢复后执行安装/升级/卸载/WebView2 手工验收。
5. M8、M9、M10、M11、M12 按依赖顺序推进；每阶段完成一次路线图复盘。
