...
- **M7.5 书源诊断**：单源重试、搜索/书籍/章节失败历史、本地保留与统计已完成首个切片；SQLite 0009 增加跨请求 `operation_id`，书源页显示任务 ID并支持导出最近 64 条脱敏失败摘要（256 KB 上限、`schema_version: 1`），CI run 31311905146 的前端检查、Rust fmt/check 和 71 个 Rust 测试通过；更细原因分类、诊断/报告版本迁移和后续兼容性指标进入 M7.5f。
...
8. M7.5 单源重试、搜索/书籍/章节失败历史、本地保留与统计、跨请求关联 ID和本地失败报告首个切片已完成（CI runs 31309322671、31309951094、31310413566、31311000040、31311905146）；下一步细化原因分类与导出版本迁移，再进入 M8。
