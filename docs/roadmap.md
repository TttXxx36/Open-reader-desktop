...
- **M7.3 XPath 评估（静态识别与离线 PoC 已完成首轮）**：导入预览在规则上下文中识别 `//`、`xpath:`、`xpath=`、`@xpath` 等表达式，最多保留 8 条、每条 512 字节并展示原始值和不执行原因；离线 PoC 只解析受限路径/谓词并统计合成 HTML 节点与估算工作量，明确拒绝函数、联合、轴、父节点和超限输入，不翻译为 CSS、不访问网络。下一步补充合成 HTML 夹具覆盖率、资源指标和更细的拒绝原因。
...
6. M7.3 XPath 静态识别与离线解析 PoC 首轮（已完成，CI run 30769462298、30770030409 通过）；下一步补充合成 HTML 夹具覆盖率与资源指标，继续禁止真实网络执行。
