...
- **M7.5 书源诊断**：单源重试首个切片已完成（`retry_source_search`、失败项“重试此源”入口、按源替换结果/诊断且保留其他结果，CI run 31309322671 通过）；失败历史持久化、重试原因分类、跨请求关联、导出版本迁移和本地统计仍待后续切片。
...
8. M7.5 单源重试首个切片已完成；下一步补失败历史、重试原因分类、跨请求诊断关联和本地统计，再进入 M8。
