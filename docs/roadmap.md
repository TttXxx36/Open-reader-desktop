...
详细执行清单见 [M8 内容格式 v2 计划](m8-content-format-plan.md)。M8.1 EPUB/TXT 导入边界已完成：EPUB ZIP 最多 2,048 个条目、单条 16 MiB、解压总量 64 MiB，并拒绝绝对/穿越路径；CI run [31322842975](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/31322842975) 通过，Rust 78 tests。下一步进入 M8.2 TXT 解析可配置化。
