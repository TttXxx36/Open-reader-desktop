...
- **M7.5 书源诊断**：单源重试、搜索/书籍/章节失败历史、本地保留与统计、跨请求 `operation_id`、本地失败报告、有限原因分类、旧库升级夹具和 `source_metrics` 观测摘要首个实现切片已完成；书源页显示任务 ID并支持导出最近 64 条脱敏失败摘要（256 KB 上限、`schema_version: 1`），CI run 31315176863 的前端检查、Rust fmt/check 和 73 个 Rust 测试通过；真实请求分母、缓存命中次数和规则成功率仍需定义采集边界。
...
8. M7.5 单源重试、搜索/书籍/章节失败历史、本地保留与统计、跨请求关联 ID、本地失败报告、有限原因分类、旧库升级、报告 schema_version 兼容设计和 source_metrics 首个实现切片已完成（CI runs 31309322671、31309951094、31310413566、31311000040、31311905146、31313787317、31314339077、31315176863）；下一步定义真实请求分母与缓存命中采集边界，再进入 M8。
