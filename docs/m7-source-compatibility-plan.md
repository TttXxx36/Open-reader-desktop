...
4. next URL opt-in 请求链的部分成功、环路、跨源、超时和超体积合成夹具已完成（67 个 Rust 测试，run 30774860219），已验证失败步骤、累计配额和阅读进度不回退；`fetch_source_chapter` 已通过可选 `next_page_policy` 接入受限方法，并复用取消 token（run 30775360305）。下一步才评估用户可见开关、取消按钮和诊断展示。
