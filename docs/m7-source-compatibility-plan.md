...
- 远程证据：[GitHub Actions run 30770030409](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/30770030409)（M7.3b，55 个 Rust 测试、前端检查通过）。

下一步是建立合成 HTML 夹具，统计常见语法覆盖率、解析耗时和资源占用，并审查是否需要更细的静态拒绝原因。只有固定节点数、表达式长度、执行时间和网络权限后，才决定是否进入可执行档；任何失败都不得静默改写为 CSS。
