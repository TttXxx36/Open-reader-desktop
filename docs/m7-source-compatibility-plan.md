...
> 维护复核（2026-08-21）：PR18 候选 `fa9a1c0` 已将书源配置本地/在线导入上限统一为 128 MiB，在线拉取超时 30 秒；候选 CI `32401610084`、Windows Release `32402250634` 和 installer smoke `32403363944` 均通过。M7.1 快照单体仍为 16 MiB、最多保留 20 个并在创建成功后自动清理旧快照；真实 Windows 窄窗口/键盘回归和 M7.2 追链开关人工验收仍待执行。当前视觉刷新不改变书源兼容等级。
...
- 本地 JSON 与 HTTPS URL 书源导入统一使用 128 MiB bundle 上限；在线拉取使用 30 秒超时和同一响应体上限，超过后明确提示，不取消结构、规则和安全校验。
...
## M7 P1 诊断与乱码降级收尾（代码切片已完成）

- `SourceRuleEvaluation` 现在是可序列化的字段级诊断模型，包含 `stage`、`rule_key`、`status` 和有限 `detail` 原因码；搜索失败、多源诊断、书籍详情、章节正文和 pipeline 都会保留它，旧缓存缺少该字段时按空数组读取。
- 书源调试页显示 pipeline 的字段诊断，脱敏诊断快照同步导出；导出仍不包含正文、关键词、Cookie、认证头或未脱敏 URL。
- 声明字符集无法可靠解码时，GB18030 回退即使没有替换字符也会标记 `encoding_warning`；书籍标题、作者和简介检测到替换字符、控制字符或常见 UTF-8 乱码标记时降级为安全空值/“未命名书籍”，并记录 `book_info.text_quality` 步骤。
- 新增回归夹具覆盖字段诊断序列化、无声明字符集的 GB18030 回退警告、乱码字段降级和诊断步骤脱敏。本地仅完成 `cargo fmt --check` 与差异检查；完整 Rust/前端验证必须由 GitHub Actions 执行。

剩余边界：需要用户授权的真实响应夹具仍不接入仓库；Windows 手工验收需确认乱码降级提示、目录/正文字段诊断和老缓存兼容，完成后再关闭 M7 P1。
