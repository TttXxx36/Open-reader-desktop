...
3. 自动追踪受限 next URL 仍未开启；纯策略闸门已固定候选校验、同源、深度/页面/响应体/总耗时、环路和稳定 stop reason，并用 60 个 Rust 测试覆盖停止原因和无限配额裁剪（run 30772265421）；累计多页夹具又证明预算不会按页重置、深度优先级稳定（61 个 Rust 测试，run 30772819136）。下一步按 [M7.2 next URL 自动追链策略](m7-next-url-policy.md) 建立授权多页夹具，再决定是否接入请求链。
