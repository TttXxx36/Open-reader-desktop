...
4. next URL opt-in 请求链的部分成功、环路、跨源、超时和超体积合成夹具已完成（67 个 Rust 测试，run 30774860219），已验证失败步骤、累计配额和阅读进度不回退；`fetch_source_chapter` 已通过可选 `next_page_policy` 接入受限方法，并复用取消 token（run 30775360305）。设置页首轮用户开关、配额控件、同源限制、远端状态/停止原因展示和缓存绕过已完成（run 31302166671）；下一步进行 Windows 手工验收，并评估更细的诊断导出字段。
