...
请求 URL 也支持每个阶段最多 8 个 `||` 候选；失败候选会进入诊断步骤，首个成功 URL 作为后续相对章节链接的基准。URL 链仅做模板渲染和请求重试，不执行脚本。每个阶段共享受限时间预算（最多 60 秒，按客户端请求超时和候选数量计算）；预算耗尽会取消当前请求、记录超时诊断并停止继续尝试。端到端书源调试、远端书籍/章节刷新和多源搜索均支持显式用户取消，整个搜索→详情→目录→正文 pipeline 另有总预算（最多 120 秒）。嵌套 URL 中间模型仍待补齐。
