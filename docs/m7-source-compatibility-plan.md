...
请求 URL 也支持每个阶段最多 8 个 `||` 候选；失败候选会进入诊断步骤，首个成功 URL 作为后续相对章节链接的基准。URL 链仅做模板渲染和请求重试，不执行脚本。每个阶段共享受限时间预算（最多 60 秒，按客户端请求超时和候选数量计算）；预算耗尽会取消当前请求、记录超时诊断并停止继续尝试。端到端书源调试还支持显式用户取消，书籍/章节刷新、多源搜索和嵌套 URL 中间模型仍待补齐。
...
3. 在不执行脚本的前提下补多级 URL 中间模型，限制深度和响应数；将阶段预算扩展为可观测的 pipeline 总耗时。
