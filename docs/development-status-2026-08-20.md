...
| M7 书源兼容性 v2 | 核心代码完成，P1 两轮通过 CI，收尾待验收 | item 自身节点匹配、响应字符集识别、目录章节链接回退和正文安全 CSS 回退已通过 CI 32466122037、32468240226；仍需授权响应夹具、乱码标题降级、`toc/content` 字段级差异诊断，以及窄窗口/键盘批量操作与拒绝边界真实提示回归 |
...
5. **M7 书源兼容性 P1 收尾**：两轮 item/编码/安全回退切片已通过 CI 32466122037、32468240226；继续补授权响应夹具、乱码标题降级和 `toc/content` 字段级差异诊断，保留 XPath、JavaScript、Cookie、Authorization、音频的显式拒绝。
...
下一步依赖顺序调整为：**M7 书源兼容性 P1 收尾（授权夹具/乱码降级/字段级诊断）→ M9.3.1-e Windows 撤销与旧 ID 验收 → M7/M8 横向收尾 → M10 备份/恢复**。Windows 安装、升级、字体、焦点和窄窗口仍需用户目标环境继续补充证据。
