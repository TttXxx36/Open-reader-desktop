...
- 当搜索或目录规则被保留为 Legacy、或页面结构导致规则没有产出时，执行器只会从同一条目中的普通 `a[href]` 链接和常见标题/字段名做安全回退；JavaScript、XPath、模板、脚本链接和认证头仍不会执行。
- JSON 书源的搜索、详情和目录在规则缺失或不匹配时，可从 `title`、`name`、`bookName`、`author`、`bookAuthor`、`url`、`href`、`link` 等固定字段回退；回退只用于提高兼容性，不改变原始规则的安全边界。
