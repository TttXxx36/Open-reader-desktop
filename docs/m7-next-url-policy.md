...
诊断快照在保持脱敏与 256 KB 上限的前提下，新增 `next_page_policy` 对象：包含 enabled、最大深度、最大页数、总响应体、总耗时、同源开关、稳定 stop reason 以及本地化状态文案。该对象不包含后续 URL、正文、关键词、书籍/章节 ID、Cookie 或认证头；旧快照读取仍可将该字段视为缺省。

远程证据：[GitHub Actions run 31302984553](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/31302984553)（前端 typecheck/build/UI contract、Rust fmt/check/test 通过）。Windows 安装包手工验收仍待执行。
...
1. 已用合成 HTTP 夹具验证成功的三页 opt-in 链，以及部分成功、环路、跨源、超时和超体积边界；取消 token 已接入 opt-in 方法和 Tauri 命令，并由远端 CI 验证。
2. Rust 单元/集成测试覆盖所有 stop_reason，并证明总响应体与总耗时预算不会被单页重置；外层取消 token、UI 取消按钮和设置开关均已接入。
3. 前端明确显示“自动追链已关闭/已启用、当前深度和停止原因”，诊断导出追加策略安全元数据且不包含正文、Cookie 或认证头（run 31302984553）。
