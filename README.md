...
- [x] 完成书源配置导入/导出与远程章节强制刷新
- [ ] 完成替换规则、章节更新、缓存容量策略和发布流程
...
M2 支持导入 TXT/EPUB、章节目录、阅读进度和字体/行距/主题设置。M3 增加书源 JSON 校验、HTML/JSON 提取器与受限 HTTP 预览；M4/M4.1 将搜索、详情、目录和首章正文串成可测试链路，并加入书源持久化、启停和调试诊断；M5 已支持启用书源并发搜索、标题/作者去重、单源失败隔离，以及搜索结果进入远程详情、目录和章节阅读。远程详情与章节正文使用 SQLite TTL 缓存，应用启动时会清理过期缓存；书源页支持经过校验的配置 JSON 导入/导出，远程阅读支持手动强制刷新。替换规则、章节更新和缓存容量策略仍在后续迭代。详细检查命令见 [docs/development.md](docs/development.md)，流程说明见 [docs/source-pipeline.md](docs/source-pipeline.md)。
