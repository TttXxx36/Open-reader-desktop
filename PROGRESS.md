...
## 2026-08-21 M9.3.1-d3 撤销与旧 ID 兼容

1. 已新增 `0018_book_merge_undo.sql`，为合并操作增加撤销状态和状态索引；撤销窗口默认 7 天，过期操作只保留审计记录，不自动清理原始数据。
2. 已实现事务化撤销：校验 canonical/来源快照和阅读状态未被外部修改，删除本次追加章节，恢复 canonical 阅读元数据、来源生命周期和单跳 alias；任何冲突都拒绝覆盖并保留现状。
3. 已实现旧书籍 ID 的单跳解析与 alias 环/多跳拒绝，并将详情、章节正文和进度保存统一到 canonical ID。
4. 新增成功撤销、重复/过期撤销、外部修改拒绝和 alias 环检测夹具；修复事务提交前释放查询语句的 Rust 借用问题。远程 CI [32470836506](https://github.com/TttXxx36/Open-reader-desktop/actions/runs/32470836506) 的前端、Linux Rust、Windows 编译/峰值 RSS 采样全部通过；本轮仍未在本地构建或安装。

## 2026-08-21 M7 P1 诊断与乱码降级收尾切片

1. 书源搜索失败、书籍详情、章节正文和端到端调试结果现在对外提供字段级 `stage/rule_key/status` 诊断；`no_match`/`rule_error` 仅使用有限原因码，不透传响应正文或认证信息。
2. 远端书籍/章节响应和多源搜索诊断会保留这些规则评估，前端调试页展示详情，脱敏诊断快照同步导出，旧缓存缺少字段时按空数组兼容读取。
3. 编码回退（包括无声明字符集时选择 GB18030）现在明确标记 `encoding_warning`；标题、作者和简介检测到替换字符、控制字符或常见 UTF-8 乱码序列时安全降级，并记录 `book_info.text_quality` 脱敏步骤。
4. 新增字段诊断序列化、GB18030 回退警告和乱码降级夹具；本地已通过 `cargo fmt --check` 与差异检查，完整 Rust/前端依赖下载受本机网络凭据限制，待 GitHub Actions 验证。
...
- P1：M7 书源兼容性剩余收尾（真实授权响应夹具仍待用户授权；乱码标题降级与 `toc/content` 字段级诊断代码已完成，待远程 CI 和 Windows 手工回归）；M9.3.1-d3 代码切片已完成，仍需目标 Windows 手工验收后进入 d3-e；M10、M11、M12 依次后置。

## 关键问题

目标 Windows 的安装升级、WebView2、字体、焦点、窄窗口和签名仍需持续人工记录；本轮 Computer Use 已补齐 0.2.0 搜索/书架、阅读恢复、设置和书源预览证据。M9.3.1-d2/d3 已通过远程 CI，仍禁止物理删除和静默覆盖；撤销仅在 7 天窗口内、快照未被外部修改时执行。详细处理顺序以状态审计文档为准。
...
4. M9.3.1-d3 代码切片已实现 7 天撤销、旧 ID 单跳跳转、外部修改冲突和 alias 环检测；下一步是目标 Windows 手工验收与 d3-e 收尾，继续禁止物理删除和静默覆盖。
