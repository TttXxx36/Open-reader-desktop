...
3. 当前下一个阻塞项仍是目标 Windows 手工验收，需按 [windows-manual-acceptance-template.md](docs/windows-manual-acceptance-template.md) 记录 PR10 搜索/书架、安装升级、书源导入、阅读恢复、字体/焦点/窄窗口和错误提示。
4. 在人工验收记录完成前，不执行 M9.3.1-d2 的 `0016` 数据库迁移、物理删除或不可撤销合并；验收完成后先进行迁移评审，再实现纯文本、无正文冲突、单事务可回滚的首版。
