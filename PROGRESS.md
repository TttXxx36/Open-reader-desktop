...
- P1：M7 书源兼容性剩余收尾（真实授权夹具、乱码标题降级展示、`toc/content` 规则差异诊断）和 M9.3.1-d3 撤销设计；M10、M11、M12 依次后置。
...
3. P1 书源兼容性首轮已完成：响应编码识别和 `item` 自身节点匹配已由 CI 32466122037 验证；下一步补充授权合成夹具、乱码标题降级提示和 `toc/content` 规则差异诊断。
