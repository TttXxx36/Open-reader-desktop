      ...
      <div v-if="sourcePanel === 'library'" class="source-grid source-library-grid">
        <section class="source-library">
          ...
          <div v-if="selectedSourceIds.length" class="source-batch-bar">
            ...
            <input v-model="sourceBatchWeight" class="source-batch-metadata-input" :disabled="sourceBatchBusy" type="number" step="1" placeholder="批量权重" aria-label="批量权重" />
            <input v-model="sourceBatchComment" class="source-batch-metadata-input" :disabled="sourceBatchBusy" placeholder="批量备注" aria-label="批量备注" />
            <button class="source-link-button" type="button" :disabled="sourceBatchBusy || (!sourceBatchWeight.trim() && !sourceBatchComment.trim())" @click="applySourceBatch('metadata')">保存备注/权重</button>
            <button class="source-link-button danger" type="button" :disabled="sourceBatchBusy" @click="applySourceBatch('delete')">批量删除</button>
          </div>
          <p v-if="selectedSourceIds.length" class="source-batch-note">可拖拽书源行调整同一分组内的自定义顺序；跨分组拖拽会被拒绝。</p>
          ...
          <div v-else class="source-list">
            <article
              ...
              @dragover.prevent
              @drop.prevent="dropSourceDrag(source.id)"
              @dragend="clearSourceDrag"
